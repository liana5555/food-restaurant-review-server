...
const deleteUserReservations = (req, res) => {
  ...
  db.query(q, [reservationId, req.userInfo.id], (err, data) => {
    if (err)
      return res.status(403).json("You can delete only your reservations.");
...
const updateReservationStatusByUser = (req, res) => {
  ...
  db.query(
    q,
    [req.body.status, req.params.id, req.userInfo.id],
    (err, data) => {
...
const getAllUsers = (req, res) => {
  ...
  db.query(q, [req.userInfo.id], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0)
      return res.status(403).json("Only admin can get others information");
...
const updateManagedUser = (req, res) => {
  ...
  db.query(q, [req.userInfo.id], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0)
      return res.status(403).json("Only admin can change others profile");
...
async function updateManagedUserRestaurantWorker(req, res) {
  ...
  if (rows.length === 0) {
    ...
    const [rows2, fields2] = await connection.execute(q2, [
      req.body.restaurant_name,
      req.body.city,
      req.body.adress,
    ]);
...
const getAllReports = async (req, res) => {
  try {
    ...
    if (result.length === 0)
      return res.status(403).json("Only admin can list reported posts");
...
const getAllReportsForPost = async (req, res) => {
  try {
    ...
    const [isAdmin] = await db.query(q, [req.userInfo.id]);
    if (isAdmin.length === 0)
      return res.status(403).json("Only admin can list reported posts");
...
const deleteReportsByPost = (req, res) => {
  ...
  db.query(q, [req.userInfo.id], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0)
      return res.status(403).json("Only admin can delete reports.");
...
const getAllReservationsByRestaurant = (req, res) => {
  ...
  db.query(q, [req.userInfo.id, req.params.restaurant_id], (err, data) => {
    ...
    if (data.length === 0)
      return res
        .status(403)
        .json("Only the restaurant's worker can get the data.");
...
const updateReservationStatus = (req, res) => {
  ...
  db.query(q, [req.userInfo.id, req.params.restaurant_id], (err, data) => {
    ...
    if (data.length === 0)
      return res
        .status(403)
        .json("Only the restaurant's worker can modify the data.");
